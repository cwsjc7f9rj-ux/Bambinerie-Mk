...
const products = [
    {
        id: 1,
        name: "Ensemble 3 Piéces",
        price: 5000,
        oldPrice: 8000,
        image: "IMG_1163.jpeg?v=5",
        category: "nouveau",
    ...
    {
        id: 3,
        name: "Ensemble. gorgeous",
        price: 6500,
        oldPrice: 8500,
        image: "IMG_1169.jpeg?v=5",
        category: "fille",
...
// Debug: afficher les prix chargés (utile pour vérifier le cache navigateur)
console.log('script.js chargé — prix des produits:', products.map(p => ({ id: p.id, price: p.price })));
...
function setupEventListeners() {
    ...
    const quickInput = document.getElementById('quick-access-input');
    if (quickInput) {
        quickInput.addEventListener('keydown', handleQuickAccess);
    }
}
...
function processQuickAccess() {
    const quickInput = document.getElementById('quick-access-input');
    if (!quickInput) return;

    const value = quickInput.value.trim().toLowerCase();
    quickInput.value = '';

    if (value === 'wave') {
        document.getElementById('wave').checked = true;
        openWaveAccount();
    } else if (value === 'orange' || value === 'orange money') {
        document.getElementById('orange').checked = true;
        openOrangeAccount();
    } else {
        showNotification('Tapez wave ou orange pour payer directement.', 'error');
    }
}

function openWaveAccount() {
    window.location.href = 'tel:+221776045398';
    showNotification('Ouverture de Wave pour payer...');
}

function openOrangeAccount() {
    window.location.href = 'tel:+221700000000';
    showNotification('Ouverture d’Orange Money pour payer...');
}
...
function updateCartUI() {
    ...
    if (cartCountSummary) {
        cartCountSummary.textContent = count;
    }
...
function processPayment() {
    ...
    if (paymentMethod === 'wave') {
        ...
        handleOrangeMoneyPayment(total, orderSummary);
...
function handleWavePayment(total, orderSummary) {
    window.location.href = 'tel:+221776045398';
    showNotification('Ouverture de Wave pour payer le compte 776045398...');
...
function handleOrangeMoneyPayment(total, orderSummary) {
    window.location.href = 'tel:+221700000000';
    showNotification('Ouverture de Orange Money pour payer le compte...');
    setTimeout(clearCheckout, 1500);
}
